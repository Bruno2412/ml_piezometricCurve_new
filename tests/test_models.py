...
class TestARIMAModel:
    def test_fit_predict_without_covariates(self):
        ...
        future_dates = pd.date_range(
            df["date"].iloc[-1] + pd.DateOffset(months=1), periods=steps, freq="MS"
        )
...
class TestFitPredict:
    ...
    def test_unknown_model_name_raises_value_error(self):
        df = _make_series_df(n=36)
        future_dates = pd.date_range(
            df["date"].iloc[-1] + pd.DateOffset(months=1), periods=3, freq="MS"
        )
    ...
    def test_auto_detects_aux_columns_from_dataframe(self):
        df = _make_series_df(n=36, with_aux=True)
        future_dates = pd.date_range(
            df["date"].iloc[-1] + pd.DateOffset(months=1), periods=4, freq="MS"
        )
...
class TestComputeCorrelationMatrix:
    def test_returns_matrix_and_count_for_sufficient_data(self):
        ...
        loaded = {
            ...
            2: {
                "df": df[["date", "level"]].assign(level=lambda d: d["level"] * 1.5),
                "name": "PZ2",
            },
    ...
    def test_returns_none_when_insufficient_overlap(self):
        df1 = pd.DataFrame(
            {"date": pd.date_range("2024-01-01", periods=1, freq="MS"), "level": [1.0]}
        )
        df2 = pd.DataFrame(
            {"date": pd.date_range("2024-01-01", periods=1, freq="MS"), "level": [2.0]}
        )
