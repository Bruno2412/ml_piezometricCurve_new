...
POINT_ALIASES = [
    "identifiant national bss",
    "ancien code national bss",
    "nom point",
    "point",
    "code bss",
    "nom_ouvrage",
...
def parse_multi_piezo_excel(df_raw, min_points=3):
    ...
    if df_raw.shape[1] < 3:
        raise ValueError(
            "Le fichier doit contenir au moins 3 colonnes : date / niveau / nom du point."
        )
    ...
    df = (
        df.dropna(subset=["date", "level", "point"])
    ...
    if len(points) < min_points:
        raise ValueError(
            f"Seuls {len(points)} point(s) distinct(s) détecté(s) — {min_points} minimum requis."
        )
...
def future_steps(freq, years):
    return {"D": years * 365, "W": years * 52, "MS": years * 12, "QS": years * 4, "YS": years}.get(
        freq, years * 12
    )
...
def draw_nappe_2d_figure(Q, S, K, thickness, distance, time_days, Area, niveau_base=0.0):
    ...
    ax.plot(
        distance, 0, "^", color="#ffc107", ms=10, zorder=8, label=f"Piézomètre ({distance:.0f} m)"
    )
    ax.annotate(
        f" Piézo\n {distance:.0f} m",
        (distance, 0),
    ...
    ax.set_aspect("equal")
    ax.set_title(
        f"Simulation Theis — t={time_days:.0f}j  Q={Q:.1f} m³/j  K={K:.1e} m/s", fontsize=10
    )
...
def response_curve_data(Q, S, K, thickness, distance, Area, t_max, niveau_base):
    ...
    impact_t = np.array(
        [calculate_spatial_impact(Q, S, K, thickness, distance, t, Area) for t in t_arr]
    )
    ...
    def fit(self, y):
        ...
        if t == "1":
            ...
            m = ExponentialSmoothing(
                y, trend=None, seasonal="add" if sp else None, seasonal_periods=sp
            )
        else:
            m = ExponentialSmoothing(
                y, trend="add", seasonal="add" if sp else None, seasonal_periods=sp
            )
...
def fit_predict(
    df_fit, steps, future_dates, model_name, freq, ci_level, ets_type="4", n_bootstraps=200
):
...
def parse_masses_eau(path):
    ...
    if not required.issubset(df.columns):
        raise ValueError(
            f"Colonnes attendues introuvables ({required}). Colonnes lues : {list(df.columns)}"
        )
    ...
    for bss_id, group in df.groupby("Identifiant national BSS"):
        ...
        if not bss_id:
            continue
        best = group.sort_values(["_qualite_rank", "_date_asso"], ascending=[False, False]).iloc[0]
...
def build_piezo_surface(coords, values, n=120, margin_ratio=0.3):
    ...
    grid_lon = np.linspace(
        lons.min() - span_lon * margin_ratio, lons.max() + span_lon * margin_ratio, n
    )
    grid_lat = np.linspace(
        lats.min() - span_lat * margin_ratio, lats.max() + span_lat * margin_ratio, n
    )
