...
def model_rf(levels, steps, points_per_year, rainfall=None):
    ...
    last_values = levels[-points_per_year:].values.tolist()
    preds = []

    for _ in range(steps):
        features = last_values[-points_per_year:]
        if rainfall is not None:
...
def model_xgb(levels, steps, points_per_year, rainfall=None):
    df = create_features(levels, rainfall)
    X = df.drop(columns=["y"])
    y = df["y"]
...
def plot_result(dates, levels, prediction, model_name, years_validation, points_per_year):

    plt.figure(figsize=(14, 6))

    val_points = int(years_validation * points_per_year)
    val_points = min(val_points, len(prediction), len(levels))

    start_val_date = dates.iloc[-val_points]

    prediction_dates = pd.date_range(start=start_val_date,
                                     periods=len(prediction),
                                     freq='MS')  # ✅ correction

    # Courbe observée
    plt.plot(dates, levels, label="Observé (Historique)",
             color='tab:blue', alpha=0.6)

    # Courbe prédiction
    plt.plot(prediction_dates, prediction,
             linestyle='--', color='tab:red', linewidth=2,
             label=f"Prédiction {model_name}")

    # Zone validation
    plt.axvspan(start_val_date, dates.iloc[-1],
                color='gray', alpha=0.2,
                label=f"Zone de Validation ({years_validation} ans)")

    # --- ALIGNEMENT ROBUSTE ---
    prediction_series = pd.Series(prediction.values, index=prediction_dates)

    common_index = prediction_series.index.intersection(levels.index)

    y_reel_validation = levels.loc[common_index]
    y_pred_validation = prediction_series.loc[common_index]

    # --- MAPE ---
    mape = np.mean(np.abs((y_reel_validation - y_pred_validation) /
                          y_reel_validation)) * 100

    # Affichage score
    color_box = 'green' if mape <= 5 else 'red'
    plt.text(0.02, 0.95, f"MAPE Validation: {mape:.2f}%",
             transform=plt.gca().transAxes,
             fontsize=12, fontweight='bold',
             bbox=dict(facecolor=color_box, alpha=0.8))

    plt.legend(loc='upper left')
    plt.grid(True, linestyle=':', alpha=0.5)
    plt.title(f"Validation et Prédiction : {model_name}")
    plt.xlabel("Années")
    plt.ylabel("Niveau")

    plt.show()

    print(f"Le modèle {model_name} présente une erreur de {mape:.2f}% sur la période de validation.")


# def plot_result(dates, levels, prediction, model_name, years_validation, points_per_year):
#
#     plt.figure(figsize=(14,6))
#     plt.plot(dates, levels, label="Observé", color='tab:blue')
#
#     #months = years_validation * 12
#     #cutoff = dates.iloc[-months]
#     validation_points = int(years_validation * 12)
#     cutoff = dates.iloc[-validation_points * points_per_year]
#     print(cutoff)
#
...
def run_model(years_to_predict=5,
    ...
    root = tk.Tk()
    root.withdraw()
    #ttk().withdraw() #cache la fenêtre principale Tkinter
    ...
    dates = pd.to_datetime(df.iloc[:,4])
    levels = df.iloc[:,6].astype(float)

    dt_days = (dates.iloc[1] - dates.iloc[0]).days
    points_per_year = int(365 / dt_days)

    rainfall = None
    #steps = years_to_predict * 12


    steps = years_to_predict * points_per_year
    ...
    if model_choice == "ARIMA":
        ...
        if ets_option == "1":
            ...
            prediction = model_ets(levels, steps)
            prediction = apply_hydro_with_relaxation(prediction, levels, storage,
                                                     recharge_factor, recharge_maitrisee,
        ...
        prediction = model_rf(levels, steps, points_per_year, rainfall)
        name = "RandomForest"

    elif model_choice == "XGBoost":
        prediction = model_xgb(levels, steps, points_per_year, rainfall)
        name = "XGBoost"
    ...
    plot_result(dates, levels, prediction, name, years_validation, points_per_year)
