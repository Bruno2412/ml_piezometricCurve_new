        ...
        if len(set(selection)) < 3:
            ...
            if ok:
                ...
                with tab_carte:
                    ...
                    # Sélection du fichier descriptif ADES
                    descriptif_file = st.file_uploader(
                        "Sélectionner le fichier descriptif ADES",
                        type=["txt"]
                    )
                    ...
                    if descriptif_file is not None:
                        ...
                        try:
                            # Création d'un fichier temporaire contenant le fichier uploadé
                            import tempfile
                            import os
                            ...
                            # Lecture du fichier avec ta fonction existante
                    ...
                    missing = [
                        ...
                        if name.strip() not in coords_dict
                    ]
                
                    available = [
                        name for name in selection
                        if name.strip() in coords_dict
                    ]
                    ...
                    if len(available) < 3:
                        ...
                        idx_available = [
                            selection.index(name) + 1
                            for name in available
                        ]
                
                        min_date = max(
                            chronicles[i]['df']['date'].min()
                            for i in idx_available
                        )
                
                        max_date = min(
                            chronicles[i]['df']['date'].max()
                            for i in idx_available
                        )
                        ...
                        if min_date >= max_date:
                            ...
                            st.caption(
                                f"⚠️ Interpolation linéaire entre "
                                f"{len(available)} point(s) — la surface colorée "
                                "n'est valide qu'à l'intérieur du polygone formé "
                                "par les points disponibles, et reste une "
                                "approximation grossière comparée à un krigeage "
