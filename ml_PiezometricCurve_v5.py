...
def run_model(years_to_predict, recharge_maitrisee, pumping, years_validation, model_choice, ets_option):
    ...
    try:
        ...
        # --- 7. CALCUL DU MAPE ---
        # Comparaison sur la période de validation uniquement
        val_idx_limit = int(years_validation * points_per_year)
        pred_val = prediction[:val_idx_limit]
        obs_val = y_true_val[:len(pred_val)]
        erreurs = obs_val - pred_val
        std_erreur = np.std(erreurs)
        mape = calculate_robust_mape(y_true_val, pred_val)

        # --- 7. CALCUL DE L'INCERTITUDE SÉCURISÉ ---
        val_idx_limit = int(years_validation * points_per_year)
        pred_val = prediction[:val_idx_limit]

        # On récupère les valeurs réelles sur la même période
        obs_series = y_true_val.values

        # On s'assure que les deux listes ont exactement la même taille
        min_len = min(len(pred_val), len(obs_series))
        y_true_final = obs_series[:min_len]
        y_pred_final = pred_val[:min_len]

        # CALCUL DE L'ERREUR (en ignorant les valeurs vides éventuelles)
        erreurs = y_true_final - y_pred_final
        std_erreur = np.nanstd(erreurs)  # nanstd ignore les valeurs vides (NaN)

        # Si std_erreur est toujours NaN ou nul par erreur, on met une valeur par défaut
        if np.isnan(std_erreur):
            std_erreur = 0.0


        #mape = calculate_robust_mape(y_true_val, prediction[:val_idx_limit])

        # --- 8. AFFICHAGE FINAL ---
        # On passe toutes les variables à plot_result_v2 pour l'affichage des paramètres
        plot_result_v2(
