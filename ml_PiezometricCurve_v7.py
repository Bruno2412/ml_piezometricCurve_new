...
def apply_hydro_model(prediction, storage, recharge_factor, recharge_maitrisee, pumping, hydro_strength,
                      points_per_year, last_obs_dates):
    # On récupère les valeurs et les dates
    adjusted = prediction.values.copy()
    dates = prediction.index

    # On identifie le point de bascule (dernier point de l'historique)
    # Dans ton flux, la prédiction commence à start_validation_date.
    # On calcule le nombre de points qui correspondent à la "Validation"
    # pour ne commencer le cumul qu'après.

    cumul_impact = 0.0
    s_val = storage if storage > 0 else 0.02

    # On définit arbitrairement que le cumul commence après la période de validation
    # ou on peut utiliser une logique de date si l'index est bien fourni.
    for i in range(len(adjusted)):
        # 1. On garde l'oscillation naturelle (f) sur TOUTE la courbe
        oscillation = recharge_factor * np.sin(2 * np.pi * i / points_per_year)

        # 2. On n'applique la recharge (cumul) que si on est dans le "Futur"
        # On détecte le futur par rapport à la date du jour (ou la fin de l'obs)
        # Ici, on simplifie : si on dépasse l'index de validation
        # (tu as défini years_validation * 12 points de validation)

        # Note : Pour être précis, on vérifie si la date actuelle est > à la date de fin réelle
        # Pour cela, on a besoin que 'prediction' soit une Series avec index Temporel

        # Si on veut rester simple et coller à ton image :
        # On peut estimer que le futur commence après 'years_validation'
        if i >= (len(adjusted) - (int(len(adjusted) / points_per_year) - 20) * 12):  # Adaptatif
            # Cette condition est complexe, utilisons plutôt la date réelle :
            pass

    # VERSION NETTOYÉE ET ROBUSTE :
    last_date_in_data = prediction.index[prediction.index <= pd.Timestamp.now()].max()

    for i in range(len(adjusted)):
        # Oscillation toujours présente
        adjusted[i] += recharge_factor * np.sin(2 * np.pi * i / points_per_year)

        # Cumul uniquement dans le futur
        if dates[i] > last_date_in_data:
...
def add_multi_year_cycle(prediction, amplitude=0.3, period_years=6):
    """
    Transforme une courbe lisse en prédiction réaliste avec
    bruit, variation d'amplitude et dérive aléatoire.
    """
    n = len(prediction)
    # On crée un vecteur d'indices pour les calculs
    x = np.arange(n)

    # 1. Cycle de base (la sinusoïde)
    cycle = amplitude * np.sin(2 * np.pi * x / (12 * period_years))

    # 2. Variation d'amplitude (le cycle n'est pas tjs aussi fort)
    # On fait varier l'amplitude de +/- 20% sur un cycle très long
    variation = 1 + 0.2 * np.sin(2 * np.pi * x / (12 * period_years * 3))
    cycle *= variation

    # 3. Bruit blanc (les petites secousses "propres" aux données réelles)
    # On prend 10% de l'amplitude comme écart-type du bruit
    noise = np.random.normal(0, amplitude * 0.1, n)

    # 4. Dérive (Random Walk) : pour que la courbe ne revienne pas
    # exactement à son point de départ comme une sinusoïde parfaite
    drift = np.cumsum(np.random.normal(0, 0.005, n))

    # On applique le tout à la prédiction d'origine
    adjusted_values = prediction.values + cycle + noise + drift

    return pd.Series(adjusted_values, index=prediction.index)
...
def run_model(years_to_predict, recharge_maitrisee, pumping, years_validation, model_choice, ets_option):
    ...
    try:
        if model_choice == "ARIMA":
            prediction_raw = model_arima(train_levels_series, steps_total)
        elif model_choice == "ETS":
            trend_type = None if ets_option == "1" else 'add'
            prediction_raw = model_ets_base(train_levels_series, steps_total, trend=trend_type)
            prediction_dates = pd.date_range(
                start=start_validation_date + pd.DateOffset(months=1),
                periods=len(prediction_raw),
                freq='MS'
            )
            prediction=pd.Series(np.asarray(prediction_raw).flatten(), index=prediction_dates)

            if ets_option == "2":
                prediction = add_multi_year_cycle(prediction, amplitude=0.4, period_years=7)
                #prediction_raw = add_multi_year_cycle(prediction_raw)

            elif ets_option == "3":
                prediction_raw = recenter_prediction(prediction_raw, train_levels_series)

            elif ets_option == "4":
                prediction_raw = apply_hydro_with_relaxation(
                    prediction_raw, train_levels_series, storage,
                    recharge_factor, recharge_maitrisee,
                    pumping, hydro_strength, points_per_year
                )

        elif model_choice == "RandomForest":
            prediction_raw = model_rf(train_levels_series, steps_total, points_per_year)
        elif model_choice == "XGBoost":
            prediction_raw = model_xgb(train_levels_series, steps_total, points_per_year)

        if prediction_raw is None: return

        # --- 5. SYNCHRONISATION DES DATES (CRUCIAL) ---
        # On crée les dates d'abord
        prediction_dates = pd.date_range(
            start=start_validation_date + pd.DateOffset(months=1),
            periods=len(prediction_raw),
            freq='MS'
        ...
        # On transforme le résultat brut en Series Pandas avec l'index temporel
        # C'est ce qui permet d'éviter l'erreur de comparaison Timestamp
        prediction = pd.Series(np.asarray(prediction_raw).flatten(), index=prediction_dates)

        # --- 6. POST-PROCESS HYDRO (AVEC GESTION DU FUTUR) ---
        if not (model_choice == "ETS" and ets_option == "4"):
            # On utilise maintenant la version de apply_hydro qui accepte last_obs_date
        ...
        # --- 7. CALCUL DU MAPE ET INCERTITUDE ---
        val_idx_limit = int(years_validation * points_per_year)
        pred_val = prediction.iloc[:val_idx_limit]  # Utilisation de iloc car c'est une Series

        # On aligne les données réelles et prédites pour le calcul
        obs_series = y_true_val.values
        min_len = min(len(pred_val), len(obs_series))

        y_true_final = obs_series[:min_len]
        y_pred_final = pred_val.values[:min_len]

        erreurs = y_true_final - y_pred_final
        std_erreur = np.nanstd(erreurs) if len(erreurs) > 0 else 0.0
        mape = calculate_robust_mape(y_true_val, pred_val.values)

        # --- 8. AFFICHAGE FINAL ---
        plot_result_v2(
            dates_historique=df_clean.index,
            levels_historique=df_clean['level'],
            prediction=prediction.values,  # On repasse en valeurs pour le plot
            prediction_dates=prediction.index,
        ...
        traceback.print_exc()  # Affiche la ligne exacte de l'erreur dans la console
...
def launch_gui():
    ...
    def on_run():
        run_model(int(vars["Années futur"].get()), float(vars["Recharge maîtrisée (m)"].get()),
                  float(vars["Pompage (m)"].get()), int(vars["Années validation"].get()),
                  vars["Modèle"].get(), vars["Option ETS (1-4)"].get())
